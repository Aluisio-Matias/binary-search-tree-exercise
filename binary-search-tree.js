...
class BinarySearchTree {
  ...
  insert(val) {
    ...
    //find the correct place to insert in the Node
  ...
  findRecursively(val, current = this.root, found = false) {
    if (val === current.val) return current;
  ...
  dfsPreOrder() {
    ...
    const traverse = (node) => {
      data.push(node.val);
      node.left && traverse(node.left); // go to the left if there's a left
      node.right && traverse(node.right); // go to the right if there's a right
    }
    traverse(current);
    return data;
  }
